...
def glue_signals_2d_iteratively(lower_signal, upper_signal, window_length=200, correlation_threshold=0.95,
    ...
    glued_signal: array
       The glued signal array, same size as lower_signal and upper_signal.
    glued_std : array
       The uncertainty of the glued signal
    gluing_center_idx: int
       Index choses to perform gluing.
    gluing_score: float
       The gluing score at the chosen point.
    c_lower, c_upper: floats
       ...
       value of `use_upper_as_reference` argument.
    """
    can_calculate_errors = (lower_std is not None) and (upper_std is not None)
    ...
    if can_calculate_errors:
        glued_std = np.empty_like(lower_signal)
    else:
        glued_std = None
    ...
    lower_std_profile = None
    upper_std_profile = None
    ...
    for profile_idx in range(profile_number):
        ...
        if can_calculate_errors:
            lower_std_profile = lower_std[profile_idx, :]
            upper_std_profile = upper_std[profile_idx, :]

        try:
            glued_profile, glued_std_profile, gluing_center_idx, gluing_score, c_lower, c_upper = glue_signals_1d(lower_profile,
                                                                                           upper_profile,
                                                                                           ...
                                                                                           lower_std=lower_std_profile,
                                                                                           upper_std=upper_std_profile)
            ...
            if skip_on_fail:
                ...
                if can_calculate_errors:
                    glued_std_profile = glued_profile
            else:
                if use_upper_as_reference:
                    glued_profile = upper_profile
                    glued_std_profile = upper_std_profile
                else:
                    glued_profile = lower_profile
                    glued_std_profile = lower_std_profile

        glued_signal[profile_idx, :] = glued_profile
        if can_calculate_errors:
            glued_std[profile_idx, :] = glued_std_profile
    ...
    return glued_signal, glued_std, center_idxs, gluing_scores, c_lowers, c_uppers
